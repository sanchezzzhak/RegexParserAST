...
class RegexParserAST
{
    ...
    private function parseIdentityEscape()
    {
        // IdentityEscape ::
        //      [+U] SyntaxCharacter
        //      [+U] /
        ...
        $l = $this->lookahead();
        if (($this->hasUnicodeFlag && preg_match("/[\^\$\.\*\+\?\(\)\\\[\]\{\}\|\/]/", $l))
            || (!$this->hasUnicodeFlag && $l !== "c")) {
        if ($l === "k" && $this->lookbehind) {
            return null;
        }
...
//        tmp = incr();
    ...
    private function parseAtomEscape($insideCharacterClass = false)
    {
        ...
        if ($insideCharacterClass) {
            ...
            if ($this->match('b')) {
                ...
                return $this->createEscaped('singleEscape', 0x0008, "\\b");
            }
            if ($this->match('B')) {
                $this->makeThrow("\B not possible inside of CharacterClass", $from, $this->pos);
            }
            if (!$this->hasUnicodeFlag && ($res = $this->matchReg("/^c([0-9])/"))) {
                // B.1.4
                // c ClassControlLetter
                return $this->createEscaped('controlLetter', $res[1] + 16, $res[1], 2);
            }
            //     [+U] -
            if ($this->match('-') && $this->hasUnicodeFlag) {
                return $this->createEscaped('singleEscape', 0x002d, "\\-");
            }
        }

        $res = $this->parseCharacterEscape();
        return $res;
    }
    ...
    private function isIdentifierPart($ch)
    {
        ...
        return $this->isIdentifierStart($ch)
            || ($ch >= 48 && $ch <= 57);         // 0..9
